...
```json
{
  ...
  "grammar": ["Each item is a B1+ grammar point, explained only in the native language."],
  "phrases": {
...
- `target_sentence` — keep exactly as provided; minor spelling fixes are allowed, but do not "correct" colloquial or non-standard structures (e.g. declarative questions like *"You attended high school at Winston Farmer?"*) — preserve them and note them in `grammar` instead.
- `native_sentence` — faithful, natural translation in the user's native language.
- `notes` — short, worthy-of-attention usage notes (can be empty `[]`). Focus on nuance, register, common pitfalls, or context. Do not flag grammatical errors or non-standard structures here — those belong in `grammar`
- `grammar` — grammar points at B1 or above, each explained only in the native language (can be empty `[]`). If a non-standard structure is grammatically incorrect but very common in spoken English, acknowledge both — note that it is non-standard, explain the correct form, and explicitly state that it is non-standard but very common in spoken English.
