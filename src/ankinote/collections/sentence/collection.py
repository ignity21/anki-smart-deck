...
class SentenceCollection:
    ...
    async def generate_and_add_note(
        ...
        The *native_sentence* is the sentence in the user's native language;
        the corresponding target sentence will be generated by the LLM.
        """
        logger.info(f"Starting generation for sentence: {native_sentence}")
        ...
        logger.success(
            f"Completed generation for sentence '{native_sentence}', note {note_id}"
        )
        return note_id
    ...
    async def _store_media_files(self, card_data: SentenceCardData) -> MediaReferences:
        ...
        sentence_model = card_data.model
        filename = hashlib.md5(sentence_model.native_sentence.encode()).hexdigest()[:12]
