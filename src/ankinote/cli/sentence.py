...
def add(sentence, native, target, llm):
    """Generate and push a single production sentence card.

    The *sentence* argument should be in the native language. The card
    front will show it as a prompt; the target-language version will be
    generated by the AI and shown on the back.
    """
...
def batch(sentences, file, native, target, llm, rpm):
    """Generate and push multiple production sentence cards.

    \b
    Sentences should be provided in the native language. They can be passed as
    arguments, read from a file (one per line), or both at the same time.

    \b
    Examples:
      anki sentence add "我今天起晚了。"
      anki sentence batch --file sentences.txt
      anki sentence batch "我今天起晚了。" --file more.txt
    """
